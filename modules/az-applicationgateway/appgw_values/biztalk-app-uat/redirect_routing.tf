locals {
  ...
  redirect_routing = {
    ...
    listeners = [
      {
        name                           = "${local.redirect_app_name}-redirect-listener"
        ...
        host_name                      = var.appgw_hostname     # This tells the gateway to catch requests for this domain
      }
    ]

    routing_rules = [
      {
        name          = "${local.redirect_app_name}-redirect-rule"
        listener_name = "${local.redirect_app_name}-redirect-listener"
        ...
        priority      = 5
        redirect_name = "${local.redirect_app_name}-redirect" # Name of the redirect configuration to use for this routing rule. This must match the name of a defined redirect configuration in the Application Gateway. The redirect configuration specifies the type of redirect (e.g., Permanent, Found) and the target URL to which traffic should be redirected.
      }
    ]

    redirects = [
      {
        name = "biztalk-uat-ip-to-fqdn"
        type = "Permanent"               # IP → FQDN
        url  = "https://uat.biztalk.com" # Target URL for the redirect. This is the URL to which incoming traffic will be redirected when this rule is matched. In this case, we're redirecting to the HTTPS endpoint of the application.
      }
    ]
